import { TAppConfig, TServerConfig } from "@apk_core/config/config.types";
...
async function bootstrap() {
	...
	if (!appCfg) throw new Error("App config is not defined");
	if (!serverCfg) throw new Error("Server config is not defined");
		...
		helmet({
			contentSecurityPolicy: {
				directives: {
					...
					// Only enable when the public URL is HTTPS; otherwise browsers will upgrade
					// Swagger UI assets to https://... and fail with ERR_SSL_PROTOCOL_ERROR.
					"upgrade-insecure-requests": appCfg.isDocker ? [] : null,
				},
			},
	...
	// listen
	await app.listen(serverCfg.port, String(serverCfg.host).trim(), () => {
