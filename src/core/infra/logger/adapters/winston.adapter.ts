import { type TLoggerConfig } from "@apk_core/config/config.types";
import { loggerConfig } from "@apk_core/config/root.config";
...
export class WinstonAdapter {
	...
	private initLogger(
		...
		if (activeLogFiles) {
			...
			transports.push(this.buildFileTransport("http", dir));
