...
export class HandlebarsTemplateAdapter implements OnModuleInit {
	...
	private registerPartials() {
		const partialsDir = path.join(this.mailerCfg.templatesDir, "partials");
	...
	public renderTemplate(templateName: string, context: Record<string, unknown>): string {
		const layoutPath = path.join(this.mailerCfg.templatesDir, "layouts", "main.hbs");
		const templatePath = path.join(this.mailerCfg.templatesDir, `${templateName}.hbs`);
